...
from soh.config import (
    ...
    load_rules, add_drop, add_fold, add_ask, clear_rule,
)
        ...
        with st.container(border=True):
            ...
            with b2:
                ...
                if st.button("Exclude (drop)", key=f"dr_{finding.item}",
                    ...
                    st.rerun()
                if st.button("Ask me every time", key=f"ak_{finding.item}",
                             use_container_width=True):
                    add_ask(finding.item)
                    st.rerun()
            st.caption(
                "**Alias** — same item, spelled differently.  "
                "**Fold** — different item, its stock always rolls into another.  "
                "**Drop** — never part of the report.  "
                "**Ask me every time** — the decision changes week to week, so "
                "flag it again next report instead of guessing."
            )
...
st.subheader("Unit accounting")

excluded_i = sum(f.intransit for f in result.findings
                 if f.category in ("unmatched", "rule") and f.level != "warning")
excluded_o = sum(f.onhand for f in result.findings
                 if f.category in ("unmatched", "rule") and f.level != "warning")
unmatched_i = sum(f.intransit for f in unmatched)
unmatched_o = sum(f.onhand for f in unmatched)

raw_i, raw_o = tw_file.item_totals()
if agt_file:
    a_i, a_o = agt_file.item_totals()
else:
    a_i = a_o = 0

acct = pd.DataFrame([
    {"Stage": "TW raw file", "Intransit": raw_i, "Onhand": raw_o},
    {"Stage": "TW into report", "Intransit": result.stats.get("tw_intransit", 0),
     "Onhand": result.stats.get("tw_onhand", 0)},
    {"Stage": "TW excluded", "Intransit": raw_i - result.stats.get("tw_intransit", 0),
     "Onhand": raw_o - result.stats.get("tw_onhand", 0)},
    {"Stage": "AGT raw file", "Intransit": a_i, "Onhand": a_o},
    {"Stage": "AGT into report",
     "Intransit": sum(v["AGT_intransit"] for v in result.values.values()),
     "Onhand": result.stats.get("agt_onhand", 0)},
])
st.dataframe(
    acct.style.format({"Intransit": "{:,.0f}", "Onhand": "{:,.0f}"}),
    use_container_width=True, hide_index=True,
)
st.caption(
    "Every unit in the raw file is either in the report or listed as "
    "excluded above. If the excluded row is not zero, check why before "
    "you submit."
)

dropped_findings = [f for f in infos
                    if f.category == "rule" and (f.intransit or f.onhand)]
if dropped_findings:
    st.markdown("**Excluded by rule**")
    st.dataframe(
        pd.DataFrame([{
            "Item": f.item,
            ...
            "Onhand": f.onhand,
            "Rule": f.message,
        } for f in dropped_findings]).style.format(
            {"Intransit": "{:,.0f}", "Onhand": "{:,.0f}"}),
        use_container_width=True, hide_index=True,
    )
    st.caption(
        "These are excluded on purpose by a saved rule. They are listed every "
        "run so a rule can never quietly remove stock without you seeing it. "
        "Remove a rule in the sidebar if it no longer applies."
    )

if unmatched_i or unmatched_o:
    st.error(
        f"**{fmt(unmatched_i)} Intransit and {fmt(unmatched_o)} Onhand will "
        "not appear in the report** because those items have no roster row. "
        "Resolve them above, or the report will be short by that amount."
    )
...
proceed_anyway = True
if unmatched:
    lost_i = sum(f.intransit for f in unmatched)
    lost_o = sum(f.onhand for f in unmatched)
    st.error(
        f"### ⚠ {fmt(lost_i)} Intransit and {fmt(lost_o)} Onhand will be lost\n\n"
        f"{len(unmatched)} item(s) have stock but no matching roster row:\n\n"
        + "\n".join(
            f"- **{f.item}** — Intransit {fmt(f.intransit)}, Onhand {fmt(f.onhand)}"
            for f in unmatched
        )
        + "\n\nGo back to step 4 and assign each one, or confirm below that "
          "you want to generate without them."
    )
    proceed_anyway = st.checkbox(
        f"I understand the report will be short by {fmt(lost_i)} Intransit "
        f"and {fmt(lost_o)} Onhand",
        value=False, key="confirm_lossy",
    )
elif errors:
    st.warning(f"{len(errors)} error(s) still open — review step 4.")
...
if st.button("📄 Generate report", type="primary", use_container_width=True,
    ...
    try:
        ...
        if unmatched:
            st.warning(
                f"Generated **without** {len(unmatched)} unmatched item(s): "
                + ", ".join(f.item for f in unmatched)
                + ". The report is short by "
                f"{fmt(sum(f.intransit for f in unmatched))} Intransit and "
                f"{fmt(sum(f.onhand for f in unmatched))} Onhand."
            )
        if reset_manual:
            st.info(
...
with st.sidebar:
    ...
    st.subheader(f"Always ask ({len(rules.get('ask', []))})")
    if rules.get("ask"):
        for item in rules["ask"]:
            s1, s2 = st.columns([4, 1])
            s1.text(item)
            if s2.button("✕", key=f"dk_{item}"):
                clear_rule(item)
                st.rerun()
    else:
        st.caption("None yet.")
